...
//создаём другие узлы графа
        do {
            ...
            children = temp;//общее число потомков для нового массива узлов
            nodeArray = tempArray;//присвоение ссылки на вспомогательный массив основному массиву узлов
            //nodesList.add(nodeArray);//заносим сформированный массив в список массивов
            temp = 0;//сбрасываем вспомогательную переменную
            if (sum >= n) {//если достигнуто заданное количество узлов обнуляем всех потомков узлов последенего массива - делаем висячими
                for(Node lastNodes: nodeArray){//перебираем последний массив
                    lastNodes.child=0;//и обнуляем потомков для каждого из его узлов
